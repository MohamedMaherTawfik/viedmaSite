    ...
    <div class="flex flex-col flex-1">
        <main class="p-6 flex-1">
            ...
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold">{{ __('main.all_students') }}</h2>
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.academicStages') }}"
                        class="border border-gray-500 text-gray-500 px-4 py-2 rounded hover:bg-gray-500 hover:text-white transition">
                ...
                <div class="overflow-x-auto overflow-visible relative z-[1]">
                    ...
                    <table class="min-w-full text-sm text-right border-separate border-spacing-y-2">
                        <thead class="bg-gray-100 text-gray-700">
                            <tr>
                                <th class="px-4 py-2 rounded-r-lg">{{ __('main.student_name') }}</th>
                                <th class="px-4 py-2">{{ __('main.email') }}</th>
                                <th class="px-4 py-2 text-center">{{ __('main.national_id') }}</th>
                                <th class="px-4 py-2 text-center">{{ __('main.nationality') }}</th>
                                <th class="px-4 py-2 text-center">{{ __('main.academic_stage') }}</th>
                                <th class="px-4 py-2 text-center">{{ __('main.parent_phone') }}</th>
                                <th class="px-4 py-2 text-center">{{ __('main.school') }}</th>
                                <th class="pr-8 pl-4 py-2 rounded-l-lg text-center">{{ __('main.actions') }}</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            @foreach ($students as $student)
                                <tr class="bg-gray-50 text-gray-800"
                                    data-edit-url="{{ route('admin.student.edit', ['student' => $student]) }}"
                                    data-delete-url="{{ route('admin.student.delete', ['student' => $student]) }}"
                                    data-link-parent-url="{{ route('admin.student.linkParent', ['name' => $student]) }}">
                                    ...
                                    <td class="px-4 py-2 flex items-center gap-2 text-center">
                                        ...
                                        {{ $student->name ?? __('main.no_name') }}
                                    </td>

                                    <td>{{ $student->me->email ?? __('main.no_email') }}</td>

                                    <td class="px-4 py-2 text-center">
                                        {{ $student->national_id ?? __('main.no_national_id') }}
                                    </td>

                                    <td class="px-4 py-2 text-center">
                                        {{ $student->nationallity ?? __('main.no_nationality') }}
                                    </td>
                                    ...
                                    <td>{{ $student->school->name ?? __('main.no_school') }}</td>
                    ...
                    <script>
                        ...
                        document.querySelectorAll('.dropdown-btn').forEach(btn => {
                            ...
                            <a href="${deleteUrl}" onclick="return confirm('{{ __('main.delete_confirm') }}');"
                            ...
                            <a href="${linkParentUrl}"
                                class="flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-100">
                                <span class="w-6 h-6 flex items-center justify-center border border-green-500 rounded-full">
