    ...
    <div class="flex flex-col flex-1">
        ...
        <main class="p-6 flex-1 space-y-8">
            ...
            <button onclick="openEditModal({{ $course->id }})"
                class="inline-block px-4 py-2 bg-[#176b98] text-white font-bold rounded-lg hover:bg-[#0f4a6d] transition">
                {{ __('main.edit_course') }}
            </button>

            <!-- المودال -->
            <div id="editCourseModal-{{ $course->id }}"
                class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white rounded-xl w-full max-w-lg p-6 relative">
                    <h2 class="text-2xl font-bold mb-4">{{ __('main.edit_course') }}</h2>

                    <form action="{{ route('admin.courses.me.update', $course->id) }}" method="POST"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" value="{{ $course->title }}"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[#176b98] focus:border-[#176b98]">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[#176b98] focus:border-[#176b98]">{{ $course->description }}</textarea>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="level"
                                ...
                                <option value="Mid" {{ $course->level == 'Mid' ? 'selected' : '' }}>
                                    Mid</option>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="file" name="cover_photo" class="w-full">
                            @if ($course->cover_photo)
                                <img src="{{ asset('storage/' . $course->cover_photo) }}"
                                    class="w-32 h-20 mt-2 object-cover rounded">
                            @endif
                        </div>

                        <div class="flex justify-end space-x-3 mt-6">
                            <button type="button" onclick="closeEditModal({{ $course->id }})"
                                class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition">
                                Cancel
                            </button>
                            <button type="submit"
                                class="px-4 py-2 bg-[#176b98] text-white rounded-md hover:bg-[#0f4a6d] transition">
                                Save
                            </button>
                        </div>
                    </form>

                    <button onclick="closeEditModal({{ $course->id }})"
            ...
            <div class="bg-white rounded-xl shadow p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
                    ...
                    <div>
                        ...
                        <p class="font-semibold">{{ $course->duration ?? __('main.not_defined') }}
                            {{ __('main.hours') }}</p>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach ($course->sessionTime as $item)
                        <div class="bg-gray-50 p-4 rounded shadow">
                            <p><strong>{{ __('main.zoom_link') }}</strong> -
                                {{ $item->date ?? __('main.not_defined') }} </p>
                                    ...
                                    <div id="feedbackModal-{{ $item->id }}"
                                        ...
                                        <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                                            ...
                                            <h2 class="text-xl font-semibold text-gray-800 mb-4 text-center">
                                                {{ __('main.add_feedback') }}</h2>

                                            <form action="{{ route('admin.feedback', $item) }}" method="POST"
                                                class="space-y-4">
                                                @csrf
                                                <div>
                                                    <label for="grade-{{ $item->id }}"
                                                        class="block text-sm font-medium text-gray-700 mb-1">{{ __('main.grade') }}</label>
                                                ...
                                                <div>
                                                    <label for="feedback-{{ $item->id }}"
                                                        class="block text-sm font-medium text-gray-700 mb-1">{{ __('main.feedback') }}</label>
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                @foreach ($course->graduationProject as $item)
                    <div class="border rounded-lg bg-gray-50 p-4 mb-4 flex justify-between items-start">
                        <div>
                            <p><strong>{{ __('main.project_title') }}:
                                </strong>{{ $item->title ?? __('main.not_defined') }}</p>
                            <p class="text-sm text-gray-600"><strong>{{ __('main.project_description') }}:
                                </strong>{{ $item->description ?? __('main.not_defined') }}</p>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($item->user && $item->user->reviews && $item->user->reviews->file)
                                        <a href="{{ $item->user->reviews->file }}"
                                            class="text-green-600">{{ __('main.download') }}</a>
                                    @else
...
<script>
    function openEditModal(id) {
        document.getElementById(`editCourseModal-${id}`).classList.remove('hidden');
    }

    function closeEditModal(id) {
        document.getElementById(`editCourseModal-${id}`).classList.add('hidden');
    }
</script>
