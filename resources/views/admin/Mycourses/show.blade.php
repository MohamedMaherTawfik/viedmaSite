            ...
            <div class="bg-white rounded-xl shadow p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
                    <div>
                        ...
                        <p class="font-semibold">{{ $course->title ?? 'غير محدد' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">وصف الدورة</p>
                        <p class="text-sm">
                            {{ \Illuminate\Support\Str::limit($course->description ?? 'غير محدد', 20, '...') }}
                        </p>
                    </div>
                    <div>
                        <p class="text-gray-500">تاريخ البدء</p>
                        <p class="font-semibold">{{ $course->start_Date ?? 'غير محدد' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">المده</p>
                        <p class="font-semibold">{{ $course->duration ?? 'غير محدد' }} ساعات</p>
                    </div>
                    <div>
                        <p class="text-gray-500">الحالة</p>
                        <p class="text-green-600 font-bold">{{ $course->status ?? 'غير محدد' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">المستوي</p>
                        <p class="text-green-600 font-bold">{{ $course->level ?? 'غير محدد' }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach ($course->sessionTime as $item)
                        <div class="bg-gray-50 p-4 rounded shadow">
                            <p><strong>Zoom Link</strong> - {{ $item->date ?? 'غير محدد' }} </p>
                            <p class="text-green-600">- {{ $item->time ?? 'غير محدد' }}</p>
                        </div>
                    @endforeach
                </div>
                    ...
                    <tbody>
                        @foreach ($uploads as $item)
                            <tr>
                                ...
                                <td>{{ $item->feedback ?? 'غير محدد' }}</td>
                                <td>{{ $item->grade ?? 'غير محدد' }}</td>
                                        ...
                                        <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                                            ...
                                            <form action="{{ route('admin.feedback', $item) }}" method="POST"
                                                ...
                                                <div>
                                                    ...
                                                    <input type="number" name="grade" id="grade-{{ $item->id }}"
                                                        required
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                @foreach ($course->graduationProject as $item)
                    <div class="border rounded-lg bg-gray-50 p-4 mb-4 flex justify-between items-start">
                        <div>
                            <p><strong>عنوان المشروع: </strong>{{ $item->title ?? 'غير محدد' }}</p>
                            <p class="text-sm text-gray-600"><strong>وصف المشروع: </strong>
