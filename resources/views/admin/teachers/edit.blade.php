             ...
             <form action="{{ route('admin.teacher.update', $teacher) }}" method="POST"
                 ...
                 <div>
                     ...
                     <input type="text" name="phone"
                         class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                         value="{{ $teacher->phone ?? 'لا يوجد بريد الكتروني' }}">
                     @error('phone')
                         <span class="text-red-500 text-sm">{{ $message }}</span>
                     @enderror
                 </div>
                 ...
                 <div>
                     ...
                     <input type="text" name="topic"
                         class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                         value="{{ $teacher->apply->topic ?? 'لا يوجد بريد الكتروني' }}">
                     @error('topic')
                         <span class="text-red-500 text-sm">{{ $message }}</span>
                     @enderror
                 </div>
                 ...
                 <div>
                     ...
                     <select name="school_id"
                         class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                         <option value="{{ $teacher->school->id ?? 'لا يوجد مدرسة' }}">
                             {{ $teacher->school->name ?? 'اختر المدرسة' }}
                         </option>
