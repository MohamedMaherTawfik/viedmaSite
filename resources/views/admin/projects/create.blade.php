<x-layout :title="__('main.trainer_dashboard')">
    ...
    <div class="flex flex-col flex-1">
        ...
        <main class="p-6 flex-1">
            ...
            <form action="{{ route('admin.project.store', $course) }}" method="POST" enctype="multipart/form-data"
                class="max-w-md mx-auto mt-10 space-y-4 p-6 bg-white rounded-lg shadow-md">
                @csrf

                <!-- Title -->
                <div>
                    <label for="title" class="block text-sm font-medium text-gray-700">{{ __('main.project_title') }}</label>
                ...
                <!-- Description -->
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700">{{ __('main.project_description') }}</label>
                ...
                <!-- Status -->
                <input type="hidden" name="status" value="{{ __('main.active') }}">

                <!-- File -->
                <div>
                    <label for="file" class="block text-sm font-medium text-gray-700">{{ __('main.project_file') }}</label>
                ...
                <div class="text-center">
                    <button type="submit"
                        class="bg-blue-600 text-white text-sm px-4 py-2 rounded-md hover:bg-blue-700 focus:outline-none">
                        {{ __('main.submit') }}
                    </button>
                </div>
            </form>
