...
<head>
    ...
    <style>
        ...
        /* CSS إضافي لجعل الفوتر في الأسفل */
        body {
            display: flex;
...
<body class="bg-white" x-data="courseLoader()" x-init="startLoading()" x-cloak>
    ...
    <main class="flex-grow">
        ...
        <!-- لازم تضيف x-data وx-init هنا علشان يشتغل AlpineJS على القسم ده -->
        <section x-data="courseLoader()" x-init="startLoading()" x-show="!isLoading" ...>
            @if ($courses->isEmpty())
                <div class="text-center text-gray-500 text-lg mt-10">
                    {{ __('messages.no_enrolled_courses') }}
                </div>
            @else
                <!-- Courses Grid -->
                <section
