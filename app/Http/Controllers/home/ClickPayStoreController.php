...
class ClickPayStoreController extends Controller
{
    ...
    public function showPaymentForm(games $game)
    {
        return view('payment.form', compact('game'));
    }

    public function initiatePayment(Request $request, games $game)
    {
        $request->validate([
        ...
        $payload = [
            ...
            "cart_amount" => $amount,
            "callback" => route('pay.callback.store', ['game' => $game]),
            "return" => route('pay.success.store', ['game' => $game]),
    ...
    public function callback(Request $request, games $game)
    {
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($result['transaction']['auth_result'] === 'A') {
                    ...
                    return redirect()->route('pay.success', ['game' => $game]);
                }
            }
    ...
    public function success(games $game)
    {
        Enrollments::create([
